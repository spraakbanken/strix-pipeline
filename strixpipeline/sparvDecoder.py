...
def createConfig(data, currentModes):
    ...
    if status:
        ...
        with open(config["settings_dir"]+"/corpora/"+data["corpus_id"]+".yaml", "w") as file:
            X = yaml.dump(corpusData, file, sort_keys=False)
...
def createMode(newModes, currentModes):
    for i in newModes:
        with open(config["settings_dir"]+"/modeStructure/" + i["name"] + ".yaml", "w") as filename:
            yaml.dump({"translation_name": {"swe": i["name"], "eng": i["name"]}}, filename, sort_keys=False)
        
        with open(config["settings_dir"]+"/modes/" + i["name"] + ".yaml", "w") as filename:
            yaml.dump({i["name"] : {"name" : i["name"], "translation_name" : {"swe" : i["name"], "eng" : i["name"]}}}, filename, sort_keys=False)

        currentModes[i["name"]] = {"name" : i["name"], "translation_name" : {"swe" : i["name"], "eng" : i["name"]}, "available" : False}
