...
class WebApiTest(unittest.TestCase):
    ...
    def test_search(self):
        result = self.do_request("/search/" + WebApiTest.corpus + "/" + WebApiTest.doc_type + "/" + WebApiTest.search["value"] + "?exclude=text,dump,lines,token_lookup")
    ...
    def test_get_document1(self):
        for doc_id in WebApiTest.doc_ids:
            result = self.do_request("/document/" + WebApiTest.corpus + "/" + WebApiTest.doc_type + "/" + doc_id + "?exclude=token_lookup")
            doc = result["data"]
    ...
    #     for doc_id in WebApiTest.doc_ids:
    #         result = self.do_request("/document/" + WebApiTest.corpus + "/" + WebApiTest.doc_type + "/" + doc_id + "?include=dump")
    #         doc = result['data']
    #         self.check_doc_text_attributes(doc)

    def test_get_documents(self):
        result = self.do_request("/document/" + WebApiTest.corpus + "/" + WebApiTest.doc_type + "/25/50?exclude=token_lookup")
