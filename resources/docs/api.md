...
#### Text query - in-text-search

Text queries are supported by: `/search`, `/related`, `/aggs` 

When `text_query_field` is empty, the given `text_query` will be 
...
* `in_order` - By setting this to false, a *keyword* search is performed. (default: true)
* `include_alternatives` - Some attributes may have many values, by default we return only the first value. (default: false)

#### Text filter - material selection

Text filters are supported by: `/search`, `/related`, `/aggs`
...
* `simple_highlight` - boolean, returns a string-highlighting with `<em>`-tags (default: false) 
* `text_query`
* `text_query_field`
* `in_order`
